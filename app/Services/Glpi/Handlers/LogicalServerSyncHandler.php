<?php

namespace App\Services\Glpi\Handlers;

use App\Services\Glpi\Contracts\SyncHandler;
use App\Services\Glpi\Mappers\LogicalServerMapper;

class LogicalServerSyncHandler implements SyncHandler
{
    public function __construct(private readonly LogicalServerMapper $mapper) {}

    public function glpiItemType(): string
    {
        return 'Computer';
    }

    public function mercatorEndpoint(): string
    {
        return 'logical-servers';
    }

    public function glpiQueryParams(): array
    {
        return [
            'range'             => '0-999',
            'expand_dropdowns'  => 1,
            'with_networkports' => 1,
            'with_devices'      => 1,
            'with_disks'        => 1,
            'with_infocoms'     => 1,
        ];
    }

    public function processOrphans(): bool
    {
        return false;
    }

    /**
     * Inclut uniquement les Computer dont le computertypes_id est dans GLPI_COMPUTER_TYPES_LOGICAL_SERVERS.
     * Si la config est vide, aucun Computer n'est synchronisé (sécurité : opt-in explicite).
     */
    public function filterItem(array $item): bool
    {
        $allowed = config('glpi.computer_types.logical_servers', []);

        if (empty($allowed)) {
            return false;
        }

        return $this->matchesComputerType($item['computertypes_id'] ?? null, $allowed);
    }

    public function map(array $glpiItem, array $context): array
    {
        return $this->mapper->map($glpiItem, $context);
    }

    private function matchesComputerType(mixed $typeValue, array $allowed): bool
    {
        if ($typeValue === null || $typeValue === 0 || $typeValue === '') {
            return false;
        }

        $typeStr = (string) $typeValue;

        foreach ($allowed as $a) {
            if (is_numeric($a) && is_numeric($typeStr) && (int) $a === (int) $typeStr) {
                return true;
            }
            if (strtolower($typeStr) === strtolower((string) $a)) {
                return true;
            }
        }

        return false;
    }
}
